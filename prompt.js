...
const getPrompt = (existingSheet, transaction, categories) => {
    ...
    console.log("existing sheet length: ", existingSheet.length)
    console.log("transaction: ", formattedTransaction)
    const prompt = (
        ...
        "Only in the case where there is no category even close, you will respond with 'misc' as the response. " +
        ...
        (existingSheet.length > 0 ? 
            ...
            : "") + 
            "Remember, you must respond with ONE category (probably ONE word only). DO NOT offer explanation or anything else in the response." +
            "Your response will be going to one cell in a spreadsheet, so it must be a single word or phrase." + 
            "Your job is only to provide the category, which will be manually checked by a human and then used as input for analysis." +
            "Again, DO Not respond with any category not in the list."
    );
